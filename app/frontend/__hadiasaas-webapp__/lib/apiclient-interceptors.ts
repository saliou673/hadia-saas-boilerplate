...
export function setupApiClientInterceptors(baseURL: string) {
  ...
  axiosInstance.interceptors.request.use((config) => {
    ...
    if (currentAccessToken) {
      nextConfig.headers.set("Authorization", `Bearer ${currentAccessToken}`);
    } else {
      nextConfig.headers.delete("Authorization");
    }
    return nextConfig;
  });
  ...
  axiosInstance.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      if (typeof window === "undefined" || status !== 401 || !originalRequest || originalRequest._retry) {
